...
static int solveLP_non_const(vector<simplex_table_row> &rows,simplex_table_row &z){
    ...
    #ifdef CASE__DEBUG
    for(int j=0;j<m;j++)assert(rows[j].b>=0.0);
    #endif
    int ret=0,iter=0;
    do{
        int _in,_out,j;
...
int main(int argc,char **argv){
    ...
    for(int j=0;j<m;j++){
        ...
        _input>>b[j]>>op;
        a[j][n+j]=-op;      // PREVIOUS SERIOUS ERROR
        ...
        if(op==-1||op==1){
            ...
            if(a[j][n+j]*b[j]<-EPS){
                ...
                for(int _j=0;_j<m;_j++){
                    if(j==_j){
                        a[_j].push_back(b[j]>EPS?1.0:-1.0);
                    }
    ...
    for(int j=0;j<m;j++){
        assert(b[j]>-EPS);
        assert(a[j][rows[j].var]==1.0);
    }
    auto s0=a[0].size(),s1=c.size(),s2=c0.size(),s3=z.a.size();
...
********************************************/
    ...
    vector<double> c1(c0);
    int ret=solveLP_non_const(rows,z_stage_1);
    // int ret=1;
    if(ret==1){
        if(-z_stage_1.b<=EPS){
            for(int j=0;j<m;j++)if(c1[rows[j].var]>0.0){
...
static int check(vector<simplex_table_row> &rows,simplex_table_row &z){
    // TODO: select the var with the minimum sigma
    int init=1;
    double min;
    for(int i=0;i<simplex_table_row::M;i++)if(z.a[i]<-EPS){   // NOTE: #0 modified '<' to change min/max
        ...
        if(j==m)return -1;      // infinete optimal solution
            ...
            if(init||z.a[i]<min||(z.a[i]<=min+EPS&&i<z.var)){
                min=z.a[i];
                z.var=i;
                if(init)init=0;
            }
...
static int select_base_out(int i,vector<simplex_table_row> &rows){
    ...
    static double min;
    int init=1;
    // TODO: if multiple min, choose the var with the least index
    for(int j=0;j<m;j++){
        if(rows[j].a[i]<=EPS)continue;
            ...
            assert(rows[j].b>-EPS);
            #endif
            if(init||threshold<min||(threshold==min&&j<index_min)){
                min=threshold;
                index_min=j;
                if(init)init=0;
            }
